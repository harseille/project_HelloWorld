...
class EditPlanCover extends Component {
  ...
  setCoverImg(e) {
    ...
    store.state = { ...store.state, coverImg: uploadFile };
    const IMG_URL = URL.createObjectURL(uploadFile); //* 파일 URL 변환
    document.querySelector('.cover').style.backgroundImage = `url(${IMG_URL})`;
  ...
  //     store.state = { ...store.state, coverImg: e.target.files[0] };

  //     reader.onload = e => { //* file을 읽으면 결과물이 인수로 들어오고 실행된다. / e.target.result (file을 url형태로 읽은 결과물)
