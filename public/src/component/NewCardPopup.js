...
class NewCardPopup extends Component {
  render() {
    ...
    const { showDatePicker, type, date, startTime, endTime, location, memo, todos } = store.state.newCardsPopup;
    const typeList = [
    ...
    // prettier-ignore
    return `<div class="dimmed__layer">
        ...
        <div class="newCard__popup__form__container memo">
          ...
          <div class="memo__container">
            <div class="memo__title">
              <input type="text" class="memo-input" placeholder="메모를 입력해주세요." name="memo" value="${memo}"/>
  ...
  addSchedule(e) {
    ...
    const { schedule, scheduleId, newCardsPopup } = store.state;
    const id = Math.max(...schedule.map(card => card.id), 0) + 1;
    store.state = {
      schedule: schedule.map((sche, i) => (i === scheduleId ? [...sche, { id, ...newCardsPopup }] : sche)),
      isShowModal: '',
    };
  }
  ...
  clickDate(e) {
    const { newCardsPopup } = store.state;

    if (e.target.matches('#newCard__date')) {
      store.state = {
        newCardsPopup: {
          ...newCardsPopup,
          showDatePicker: true,
  ...
  search(e) {
    ...
    autocomplete.addListener('place_changed', () => {
      ...
      // TODO: state처리 추가 작업 필요
    });
  }

  addTodo() {
    const { newCardsPopup } = store.state;
    const { todos } = newCardsPopup;
  ...
  addEventListener() {
    return [
      ...
      { type: 'click', selector: '.dimmed__layer', handler: this.closeModal },
      { type: 'click', selector: '.modal__header__close__btn', handler: this.closeModal },
      { type: 'change', selector: '.type__list', handler: this.changeTypeNTimeNMemo },
      { type: 'cilck', selector: '.newCard__popup__form__container .time__form__input', handler: this.clickDate },
      {
        type: 'change',
        selector: '.newCard__popup__form__container .time__form__select',
        handler: this.changeTypeNTimeNMemo,
      },
      { type: 'change', selector: '.location', handler: this.changeLocation },
